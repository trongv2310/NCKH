...
namespace DoAn_CauLong.Controllers
{
    public class SanPhamController : Controller
    {
        ...
        [HttpGet]
        public ActionResult GetGoiYSanPham(int maSP)
        {
            using (var db = new QLDN_CAULONGEntities())
            {
                var tatCaGoiY = db.GoiYSanPhams.OrderByDescending(x => x.Utility).ToList();
                var dsSanPhamGoiYIds = new HashSet<int>();

                foreach (var rec in tatCaGoiY)
                {
                    var items = rec.Itemset.Split(',').Select(int.Parse).ToList();
                    if (items.Contains(maSP))
                    {
                        foreach (var item in items)
                        {
                            if (item != maSP) dsSanPhamGoiYIds.Add(item);
                        }
                    }
                    if (dsSanPhamGoiYIds.Count >= 4) break; // Chỉ lấy 4 SP
                }

                // Nếu ko có data Mining, random 4 SP cùng loại
                if (dsSanPhamGoiYIds.Count == 0)
                {
                    var spHienTai = db.SanPhams.Find(maSP);
                    if (spHienTai != null)
                    {
                        dsSanPhamGoiYIds = new HashSet<int>(db.SanPhams
                            .Where(x => x.MaLoai == spHienTai.MaLoai && x.MaSanPham != maSP)
                            .OrderBy(x => Guid.NewGuid()).Take(4).Select(x => x.MaSanPham));
                    }
                }

                var spGoiYList = db.SanPhams.Where(sp => dsSanPhamGoiYIds.Contains(sp.MaSanPham)).ToList();
                return PartialView("_ProductRecommendations", spGoiYList);
            }
        }

        // ----------------------------------------------------
        // (Tuỳ chọn) API ẩn để bạn test chạy thuật toán thủ công 
        // ----------------------------------------------------
        [HttpGet]
        public ActionResult TestRunMining()
        {
            var service = new DoAn_CauLong.Services.MiningService();
            service.ChayThuậtToanKhaiPha(windowSize: 3, topK: 10);
            return Content("Đã chạy xong VertTopKDS và lưu vào Database!");
        }
    }
}